...
- **Shows from before the catalogue began come from setlist.fm, live, and are never stored.** `/api/shows/past` (see `lib/setlistfm.ts`) searches it as an artist, then as a venue, for the past year, and drops anything the catalogue already has. Its terms shape all of it: **non-commercial use only** (charging for Gigl needs a licence first), a followable "via setlist.fm" credit wherever its data appears, and **no copies beyond short-term caching** — so results are never written to `shows`, and a show only reaches the database inside someone's own log. `logged_shows` carries its own artist/venue/date and never reads `shows` back, which is what makes that work. The key (`SETLISTFM_API_KEY`) allows 2 requests a second and 1,440 a day; pages are cached for 6 hours and "no match" answers per instance. If the "More shows" list goes quiet, look for `[setlistfm] ... HTTP 429` in the logs.
