export function computeShowScore(performance: number, venue: number, vibe: number): number {
  return (performance + venue + vibe) / 3
}
...
export function showScore(row: RatedRow): number {
  if (row.performance_rating == null || row.venue_rating == null || row.vibe_rating == null) return 0
  return computeShowScore(row.performance_rating, row.venue_rating, row.vibe_rating)
}
